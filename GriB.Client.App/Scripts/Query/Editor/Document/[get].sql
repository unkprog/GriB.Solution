select [t].[id], [t].[doctype], [t].[option], [t].[date], [t].[salepoint], [t].[salepointto], [t].[contractor], [t].[typecost], [t].[reason]
     , [salepointname] = isnull([sp].[name], ''), [contractorname] = isnull([c].[name], ''), [reasonname] = isnull([r].[name], ''), [sum] = isnull((select sum([quantity] * [price]) from [t_document_position] with(nolock) where [id] = [t].[id]), 0)
