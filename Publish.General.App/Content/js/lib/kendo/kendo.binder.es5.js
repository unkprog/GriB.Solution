...
(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        var Binding = Observable.extend({
            init: function init(parents, path) {
                var that = this;
            ...
            start: function start(source) {
                source.bind("get", this._access);
            },

            stop: function stop(source) {
        ...
        var TemplateBinding = Binding.extend({
            init: function init(source, path, template) {
                var that = this;
        ...
        var Binder = Class.extend({
            ...
            bind: function bind(binding, attribute) {
                var that = this;
        ...
        var TypedBinder = Binder.extend({
            ...
            _parseValue: function _parseValue(value, dataType) {
                if (dataType == "date") {
        ...
        _binders.attr = Binder.extend({
            refresh: function refresh(key) {
                this.element.setAttribute(key, this.bindings.attr[key].get());
            }
        });

        _binders.css = Binder.extend({
            init: function init(element, bindings, options) {
                Binder.fn.init.call(this, element, bindings, options);
                this.classes = {};
            },
            refresh: function refresh(className) {
        ...
        _binders.style = Binder.extend({
            refresh: function refresh(key) {
                this.element.style[key] = this.bindings.style[key].get() || "";
            }
        });

        _binders.enabled = Binder.extend({
        ...
        _binders.readonly = Binder.extend({
            refresh: function refresh() {
        ...
        _binders.disabled = Binder.extend({
            refresh: function refresh() {
        ...
        _binders.visible = Binder.extend({
            refresh: function refresh() {
        ...
        _binders.invisible = Binder.extend({
            refresh: function refresh() {
        ...
        _binders.html = Binder.extend({
            refresh: function refresh() {
                this.element.innerHTML = this.bindings.html.get();
            }
        });

        _binders.value = TypedBinder.extend({
            init: function init(element, bindings, options) {
        ...
        _binders.source = Binder.extend({
            ...
            remove: function remove(index, items) {
                var idx,
        ...
        _binders.select = {
            ...
            value: TypedBinder.extend({
                init: function init(target, bindings, options) {
        ...
        var arraySplice = function arraySplice(arr, idx, remove, add) {
            add = add || [];
            remove = remove || 0;
        ...
        var BindingTarget = Class.extend({
            ...
            binders: function binders() {
                return _binders[this.target.nodeName.toLowerCase()] || {};
            },

            applyBinding: function applyBinding(name, bindings, specificBinders) {
                var binder = specificBinders[name] || _binders[name],
        ...
        var WidgetBindingTarget = BindingTarget.extend({
            binders: function binders() {
                return _binders.widget[this.target.options.name.toLowerCase()] || {};
            },

            applyBinding: function applyBinding(name, bindings, specificBinders) {
                var binder = specificBinders[name] || _binders.widget[name],
                    toDestroy = this.toDestroy,
                    attribute,
                    binding = bindings[name];
