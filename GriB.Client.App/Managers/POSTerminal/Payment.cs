...
namespace GriB.Client.App.Managers.POSTerminal
{
    ...
    {
        private static payment readFromValues(object[] values) => new payment() { id = (int)values[0], check = new check() { id = (int)values[1] }, ptype = (int)values[2], sum = (double)values[3], option = (int)values[4], client = new Models.Editor.client() { id = (int)values[5] }, comment = (string)values[6] };
        ...
        public static payment SetPayment(this Query query, payment payment, int user)
        {
            ...
            query.Execute(cmdSet, new SqlParameter[] { new SqlParameter("@id", result.id), new SqlParameter("@u", user), new SqlParameter("@check", result.check.id), new SqlParameter("@type", result.ptype), new SqlParameter("@sum", result.sum), new SqlParameter("@option", result.option), new SqlParameter("@client", result.client.id), new SqlParameter("@comment", result.comment) }
            , (values) =>
            {
