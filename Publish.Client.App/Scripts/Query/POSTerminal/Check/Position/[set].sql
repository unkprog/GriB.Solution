if exists(select [id] from [t_check_position] with(nolock) where [id]=@id and [product] = @product)
begin
  update [t_check_position] 
  ...
  insert into [t_check_position]([id], [index], [product], [quantity], [price])
