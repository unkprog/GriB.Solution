...
(function (f, define) {
    ...
    (function ($, undefined) {
        var kendo = window.kendo,
            ...
            pageSize: $.noop,
            page: $.noop,
            filter: function filter(params, _filter, useVersionFour) {
                if (_filter) {
                    _filter = toOdataFilter(_filter, useVersionFour);
                    if (_filter) {
                        params.$filter = _filter;
                    }
                }
            },
            sort: function sort(params, orderby) {
            ...
            skip: function skip(params, _skip) {
                if (_skip) {
                    params.$skip = _skip;
                }
            },
            take: function take(params, _take) {
                if (_take) {
                    params.$top = _take;
                }
            }
        ...
        extend(true, kendo.data, {
            schemas: {
                odata: {
                    ...
                    data: function data(_data) {
                        return _data.d.results || [_data.d];
                    },
                    total: "d.__count"
                }
            },
            transports: {
                odata: {
                    ...
                    parameterMap: function parameterMap(options, type, useVersionFour) {
                    ...
                    data: function data(_data2) {
                        if ($.isArray(_data2)) {
                            for (var i = 0; i < _data2.length; i++) {
                                stripMetadata(_data2[i]);
                            }
                            return _data2;
                        } else {
                            _data2 = $.extend({}, _data2);
                            stripMetadata(_data2);
                    ...
                    submit: function submit(e) {
                        ...
                        $.ajax(extend(true, {}, {
                            ...
                            error: function error(response, status, _error) {
                                e.error(response, status, _error);
                            }
                        }, options));
                    }
                }
