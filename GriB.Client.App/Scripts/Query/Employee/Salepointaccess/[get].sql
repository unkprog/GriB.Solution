select [id] = [t].[id], [name] = [t].[name]
	 ...
	 , [isaccess] = isnull([sa].[isaccess], ''), [isdefault] = isnull([sa].[isdefault], '')
from [t_org] [t] with(nolock)
left join [t_org] [t1] with(nolock) on [t].[pid] = [t1].[id]
left join [t_org_info1] [ti] with(nolock) on [ti].[id] = [t].[id]
left join (select [salepoint], [isaccess], [isdefault] from [t_employee_salepointaccess] with(nolock) where [id] = @id) [sa] on [t].[id] = [sa].[salepoint]
